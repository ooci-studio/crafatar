...
function get_hash(url) {
  logging.log(url," 16")
  return hash_pattern.exec(url)[0].toLowerCase()+'.png';
}
  ...
  networking.get_skin_info(rid, userId, profile, function(err, url, slim) {
    ...
    if (!err && url) {
      ...
      if (cache_details && cache_details.skin === skin_hash) {
        ...
        var facepath = path.join(config.directories.faces, skin_hash);
        var helmpath = path.join(config.directories.helms, skin_hash);
        var skinpath = path.join(config.directories.skins, skin_hash);
  ...
  networking.get_cape_url(rid, userId, profile, function(err, url) {
    if (!err && url) {
      ...
      if (cache_details && cache_details.cape === cape_hash) {
        ...
        var capepath = path.join(config.directories.capes, cape_hash);
