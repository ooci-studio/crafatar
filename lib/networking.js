...
var session_url = "https://auth-api.ooci.co/sessionserver/session/minecraft/profile/";
var textures_url = "https://minio-api.ooci.co/minecraft/";
...
exp.get_from_options = function(rid, url, options, callback) {
  ...
  if (is_session_req && req_count() >= config.server.sessions_rate_limit) {
    ...
    request.get({
      url: url,
      headers: {
        "User-Agent": "Crafatar (+https://ooci.co)"
      },
